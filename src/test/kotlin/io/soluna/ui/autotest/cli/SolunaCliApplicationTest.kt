...
class SolunaCliApplicationTest {
    ...
    fun `scaffold app log plugin command creates plugin project files`() {
        ...
        assertEquals(
            "io.soluna.ugreen.applog.UgreenAudioAppLogPlugin\n",
            Files.readString(
                output.resolve(
        ...
        val source = Files.readString(output.resolve("src/main/kotlin/io/soluna/ugreen/applog/UgreenAudioAppLogPlugin.kt"))
    ...
    @Test
    fun `scaffold app log plugin command requires io soluna package namespace`() {
        val output = Files.createTempDirectory("soluna-cli-plugin-scaffold-package")
        val stdout = StringBuilder()
        val stderr = StringBuilder()
        val cli = SolunaCliApplication()
        ...
        assertEquals(2, exitCode)
        assertEquals("", stdout.toString())
        assertTrue(stderr.toString().contains("--package must use the io.soluna namespace"))
    }
