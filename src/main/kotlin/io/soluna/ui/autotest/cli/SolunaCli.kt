...
import io.soluna.ui.autotest.runner.PlanRunRequest
import io.soluna.ui.autotest.runner.PlanRunResult
import io.soluna.ui.autotest.runner.PlanRunner
...
class SolunaCliApplication(
    ...
    private fun parseScaffold(args: Array<String>): CliCommand {
        ...
        val resolvedGroup = group
            ?.takeIf { it.isNotBlank() }
            ?.let { validateNamespace(it, "--group") }
    ...
    private fun validatePackageName(raw: String): String {
        return validateNamespace(raw, "--package")
    }

    private fun validateNamespace(raw: String, option: String): String {
        val value = raw.trim()
        val part = "[A-Za-z_][A-Za-z0-9_]*"
        if (!Regex("^$part(\\.$part)*$").matches(value)) {
            throw CliUsageException("$option must be a valid Kotlin package or Gradle group name")
        }
        if (value != "io.soluna" && !value.startsWith("io.soluna.")) {
            throw CliUsageException("$option must use the io.soluna namespace")
        }
        return value
    }
    ...
    private fun usage(): String {
        return """
            ...
            Scaffold:
              ...
              --package and explicit --group must use io.soluna or a child namespace.
