...
class RecoveringWebDriverAdapter(
    ...
    private val wdaManager: WdaManager? = null,
    private val wdaConfig: WdaConfig? = null,
    ...
    @Volatile
    private var currentWdaHandle: WdaHandle? = initialWdaHandle
    ...
    fun currentWdaHandle(): WdaHandle? {
        return currentWdaHandle
    }
    ...
    private fun isWdaHealthy(): Boolean {
        val handle = currentWdaHandle ?: return true
        val manager = wdaManager ?: return true
        return manager.isRunning(handle)
    }
    ...
    private fun recoverSession(
        ...
        val physicalSession = delegate.startSession(
            session.originalRequest.forRecoveredSession(
                serverUrl = currentServerHandle.url,
                webDriverAgentUrl = currentWdaHandle?.url,
            ),
        )
        session.physicalSessionId = physicalSession.sessionId
    }

    private fun recoverWdaIfNeeded() {
        val handle = currentWdaHandle ?: return
        val manager = wdaManager ?: return
        if (manager.isRunning(handle)) {
            return
        }
        val config = wdaConfig
            ?: error("WDA '${handle.url}' is not healthy but no WDA config was supplied for recovery")
        currentWdaHandle = manager.restart(handle, config)
    }

    private fun StartSessionRequest.forRecoveredSession(
        serverUrl: String,
        webDriverAgentUrl: String?,
    ): StartSessionRequest {
        if (webDriverAgentUrl == null) {
            return copy(serverUrl = serverUrl)
        }
        return copy(
            serverUrl = serverUrl,
            capabilities = capabilities + ("appium:webDriverAgentUrl" to webDriverAgentUrl),
        )
    }
