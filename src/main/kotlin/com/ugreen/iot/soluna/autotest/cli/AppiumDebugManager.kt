...
import com.ugreen.iot.soluna.autotest.appium.action.KtVisualTemplateMatcher
...
class AppiumDebugManager(
    ...
    fun runShell(
        ...
        return withDebugSession(
            ...
            stdout.appendLine("commands: source [--out file], screenshot --out file, tap --x-ratio n --y-ratio n, tap-element --strategy s --locator v, input --strategy s --locator v --text text, tap-template --template file [--roi x,y,w,h], help, exit")
            ...
            while (true) {
                ...
                val output = runCatching { executeAction(action, active.session.sessionId) }.getOrElse { err ->
