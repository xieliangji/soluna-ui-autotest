...
The older `tap: open-mine-tab` form with sibling fields remains accepted for compatibility. `case.schema.json`, `plan.schema.json`, and `fragment-catalog.schema.json` explicitly enumerate the currently supported action keywords and aliases: `tap`, `longPress`, `tapVisualTemplate`, `tapImage`, `tapTemplate`, `input`, `restartApp`, `clearAppData`, `getText`, `saveElementRect`, `wait`, `assertElementExists`, `assertElementAttrEquals`, `assertElementAttrRegexMatch`, `assertSourceRegexMatch`, `screenshot`, `startScreenRecording`, `stopScreenRecording`, and `assertScreenRecordingTextRegexMatch`. Unsupported action types fail schema/policy validation before execution.
