# soluna-ui-autotest v0 Architecture

本文档记录 `soluna-ui-autotest` 的初版架构约束和设计边界。它是后续代码骨架、DSL schema、组件 schema、插件接口和报告模型的事实来源。
...
- 测试输入资产推荐按 app id 分组，例如 `AIot-Tests/com.ugreen.iot/{plans,cases,elements,data,fragments}`；`AIot-Tests/devices` 这类设备目录保持在 app 资产根之外。
  ...
  - id: login_success
    ...
    actions:
      ...
      - input: input-username
        ...
        desc: 输入用户名
      - assertElementAttrEquals: assert-home-title
        element: home.title
...
v0 参数数据文件使用 YAML，并通过 JSON Schema 校验其 YAML 解析后的 JSON-compatible 数据结构。
...
- 用例中引用元素时使用独立字段，例如 `element: profile.nicknameInput`，不使用 `${...}` 参数引用语法。
...
v0 当前实现提供 `FailureStrategy` 接口和 `FailFastFailureStrategy` 默认实现：
...
后续可在该接口下加入继续执行、分级中断、重试后决策等策略。

v0 当前实现还提供 `RetryStrategy` 接口和两个基础实现：
...
- 默认隐式等待用于减少普通元素查找抖动。
- plan 可通过 `defaults.actionWait` 声明 action 级显式等待默认值，避免每个 action 重复写相同的 timeout / interval。
...
插件源码已拷贝到本仓库 `lib/soluna-appium-ext` 便于集成开发。该插件的上游仍是作者的 GitHub 项目 `https://github.com/xieliangji/soluna-appium-ext`。插件能力扩展需要保持可回提交到上游项目；扩展完成并验证后，应准备在插件 GitHub 项目中提交和推送。

插件能力协商、版本兼容和 schema 由本项目统一协调，但插件实现不应变成只能在主框架仓库内工作的私有分叉。
...
- 默认 action executor 覆盖 tap/input/wait/restartApp/getText/screenshot 和属性/source 断言；断言可按 `wait` 轮询。
...
trace 中的截图按普通诊断产物处理，不进入显式截图资源清单。

v0 当前 trace 截图策略：

- `trace.screenshots.enabled` 控制是否启用动作前 trace 截图。
- `beforeAction: onFailure` 在执行期只保留最近 N 张动作前截图到内存，不为通过动作落盘。
- 当 action 最终失败时，保留的动作前截图写入本地 `diagnostics/trace` 并以 `ArtifactKind.DIAGNOSTIC` 入队上传。
- 如果报告引用 trace 截图，启用 artifact uploader 时报告数据使用 MinIO URL；未启用上传时使用本地路径。
- trace 截图上传不写入 `plan-resource-manifest.json`，该 manifest 仍只面向显式截图动作资源。

用例 DSL 中显式截图动作产生的图片，需要统一整理到一个 JSON 文件中。该 JSON 主要面向其他服务或模块消费，不是步骤级执行明细。
...
- 显式截图由 `ScreenshotActionExecutor` 写入本地资源目录，再由 `PlanResourceManifestWriter` 生成 `plan-resource-manifest.json`。
- manifest 只保存计划级元信息和显式截图资源列表；不保存 action 执行明细。
...
v0 使用 JSON Schema 作为外部数据合同格式，schema 文件位于：
...
v0 已完成可运行骨架：schema-first DSL、plan-rooted runner、Android/iOS 真机 Appium 执行、managed Appium/WDA、session recovery、MinIO 上传、报告数据/HTML、显式截图 manifest、失败 trace、本地清理和 DingTalk 生命周期通知均已打通。

v0 后不再继续扩张基础说明文档。新增能力优先通过以下方式记录：

- schema 变化写入 `src/main/resources/schemas/v*/` 和 [docs/schemas.md](schemas.md)。
- 架构边界变化写入本文档对应章节。
- 每轮实现只在 [docs/progress.md](progress.md) 追加高层摘要，不记录长命令输出和完整调试过程。
- 真实业务用例产生的动作关键字、报告体验和稳定性需求进入 v1 迭代记录。
...
- `lib/soluna-appium-ext` 与上游 GitHub 项目的同步方式。
